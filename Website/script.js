...
function ready() {
    ...
    for (let i = 0; i < quantityInputs.length; i++) {
        quantityInputs[i].addEventListener('change', updateCartTotal, quantityChanged, updateGrandTotal);
    }
    ...
    document.getElementsByClassName('Btn')[0].addEventListener('click', payClicked);
}
...
function addItemToCart(img, item, price) {
    ...
    cartRow.getElementsByClassName('button')[0].addEventListener('click', removeCartItemButtons);
    cartRow.getElementsByClassName('qtn')[0].addEventListener('change', quantityChanged);
}
