...
export default function FavouritesPage() {
  ...
  function handleBulkAction(action: string) {
    ...
    switch (action) {
      case "addToPlaylist":
        ...
        break;
      case "export":
        toast.success(`Exported ${selectedSongs.length} songs to playlist`);
        break;
      case "remove":
        toast.success(`Removed ${selectedSongs.length} songs from favourites`);
        break;
    }
    setSelectedSongs([]);
  ...
  function handlePlayAll() {
    toast.success("Playing all favourites on Spotify");
    window.open("https://open.spotify.com", "_blank");
  }

  function handleShuffle() {
    toast.success("Shuffling favourites on Spotify");
    window.open("https://open.spotify.com", "_blank");
  }

  const filteredSongs = favouriteSongs.filter(
    (song) =>
  ...
  const totalDuration = favouriteSongs.reduce((acc, song) => {
      ...
      <div className="glass backdrop-blur-[1px] rounded-3xl p-8">
        <div className="flex items-start gap-6">
          ...
          <div className="flex-1">
            ...
            <p className="text-lg text-muted-foreground mb-4">
              Your curated collection of most loved tracks
            </p>
        ...
        <div className="flex items-center gap-3 mt-6">
          ...
          <Button
            size="lg"
            variant="outline"
            className={`glass backdrop-blur-xl ${isSelectMode ? "bg-primary/10 text-primary" : "hover:bg-primary/10"}`}
            onClick={() => {
        ...
        <div className="divide-y divide-border/50">
          {filteredSongs.map((song, index) => (
            ...
            >
              ...
              <div
                className={`${isSelectMode ? "col-span-1" : "col-span-1"} text-muted-foreground`}
              >
                {index + 1}
              </div>
              <div
                className={`${isSelectMode ? "col-span-4" : "col-span-5"} flex items-center gap-3`}
              >
                <Image
